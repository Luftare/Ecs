function Ecs() {
  ...
  Entity.prototype = {
    ...
    handleEnrollToSystems(preSystems = []) {
      ...
      const enteredSystems = postSystems.filter(
        system => !preSystems.includes(system)
      );
      const leftSystems = preSystems.filter(
        system => !postSystems.includes(system)
      );
    ...
    matches(system) {
      return (
        system.has.every(key => this.has(key)) &&
        system.not.every(key => !this.has(key))
      );
    },
    getMatchingSystems() {
